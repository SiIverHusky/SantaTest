...
HELP_TEXT = """LVGL Image Conversion Tool Instructions:

1. Add Files: Click "Add Files" button to select images for conversion, supports batch import

2. Remove Files: Select files by clicking the checkbox "[ ]" in the list (becomes "[√]" when selected), 
   click "Remove Selected" to delete selected files

3. Resolution: Choose desired resolution, e.g., 128x128
   Recommend selecting based on your device's screen resolution. Too large or too small will affect display quality.

4. Color Format: Choose "Auto Detect" for automatic selection based on image transparency, or specify manually
   Unless you understand this option, auto detection is recommended to avoid unexpected issues.

5. Compression Method: Choose NONE or RLE compression
   Unless you understand this option, recommended to keep default NONE

6. Output Directory: Set save path for converted files
   Default is output folder in program directory
...
class ImageConverterApp:
    ...
    def create_widgets(self):
        ...
        ttk.Button(convert_frame, text="Convert All", command=lambda: self.start_conversion(True)).pack(side=tk.LEFT, padx=5)
        ttk.Button(convert_frame, text="Convert Selected", command=lambda: self.start_conversion(False)).pack(side=tk.LEFT, padx=5)
    ...
    def on_tree_click(self, event):
        ...
        if region == "cell":
            ...
            if col == "#1":  # Clicked on selection column
    ...
    def start_conversion(self, convert_all):
        ...
        if not input_files:
            msg = "No files found for conversion" if convert_all else "No files selected"
    ...
    def convert_images(self, input_files, width, height, compress):
        ...
        print(f"Conversion complete! Success: {success_count}/{total_files} files\n")
