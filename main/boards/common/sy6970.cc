...
int Sy6970::GetBatteryLevel() {
    int level = 0;
    // Minimum voltage the battery can drop to
    ...
    // When battery is not connected, charging status is unstable and battery_voltage sometimes exceeds charge_voltage_limit
    if (level > 100) {
        level = 100;
    }
    return level;
